...
#define N_UKF 13 // number of sigma points
typedef struct {
    ...
    float sigma[N_UKF][L];     // sigma points
    float W_a[N_UKF];          // weights for mean
    float W_c[N_UKF];          // weights for covariance

    float x_pred[L];       // state predict
    float P_pred[L][L];    // covariance predict

    float z_pred[N_UKF][4];    // measurement prediction

...
void loraTask(int fd) {
    ...
    while (1) {
        ...
        {
            std::lock_guard<std::mutex> lock(mtx);
            if (wsQueue.size() < 1000)
                wsQueue.push(jsonStr);

            if (fbQueue.size() < 1000)
                fbQueue.push(jsonStr);
        }
...
void wsTask(WSServer& server) {
    ...
    while (1) {
        ...
        cv.wait(lock, [] { return !wsQueue.empty(); });

        std::string msg = wsQueue.front();
        wsQueue.pop();
...
// ================= Firebase =================
void firebaseTask(FirebaseClient& firebase) {
    while (1) {
        std::string msg;

        {
            std::unique_lock<std::mutex> lock(mtx);
            cv.wait(lock, [] { return !fbQueue.empty(); });
        ...
        int ret = sscanf(msg.c_str(),
            "{\"id\":%d,\"observer_lat\":%lf,\"observer_lon\":%lf,\"observer_alt\":%lf,"
            "\"target_lat\":%lf,\"target_lon\":%lf,\"target_alt\":%lf,\"distance\":%*lf}",
            &id, &olat, &olon, &oalt,
            &tlat, &tlon, &talt
        );
...
int main() {
    ...
    FirebaseClient firebase("https://binnocular-default-rtdb.asia-southeast1.firebasedatabase.app/tracking_history.json");

    std::thread t1(loraTask, fd);
    std::thread t2(wsTask, std::ref(server));
    std::thread t3(firebaseTask, std::ref(firebase));
    std::thread t4(networkTask);
